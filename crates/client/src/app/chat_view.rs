...
pub(super) fn add_chat_panel(
    ...
    let toggle = commands
        .spawn((
            ...
            ImageNode::new(if chat.open {
                assets.chat_close_icon.clone()
            } else {
                assets.chat_open_icon.clone()
            })
    ...
    let voice_icon = commands
        .spawn((
            ...
            ImageNode::new(assets.quick_voice_icon.clone()),
            FocusPolicy::Pass,
        ))
        .id();
...
pub(super) fn add_developer_hand_input(
    commands: &mut Commands,
    parent: Entity,
    ...
    let text = add_text(
        commands,
        field,
        developer_hand_input_label(input),
        14.0,
        if input.value.is_empty() { MUTED } else { TEXT },
        assets,
    );
    commands.entity(text).insert(DeveloperHandInputText);
}
