...
pub fn sync_play_effect(
    ...
    if let Some((_, play)) = model.last_play_effect() {
        let sounds = match card_play_sound_kind(&play.kind) {
            CardPlaySoundKind::Place => &assets.audio.place_sounds,
            CardPlaySoundKind::Shove => &assets.audio.shove_sounds,
        };
        if !sounds.is_empty() {
...
fn card_play_sound_kind(kind: &QiGuiPlayKind) -> CardPlaySoundKind {
    match kind {
        QiGuiPlayKind::Straight { .. }
        | QiGuiPlayKind::ConsecutivePairs { .. }
        | QiGuiPlayKind::Airplane { .. } => CardPlaySoundKind::Shove,
        ...
        | QiGuiPlayKind::Bomb(_)
        | QiGuiPlayKind::HeavenBomb => CardPlaySoundKind::Place,
    }
}
...
pub fn advance_play_effect(
    ...
    let duration = match active.play.kind {
        QiGuiPlayKind::HeavenBomb => 2.25,
        QiGuiPlayKind::Bomb(_) => 1.35,
        QiGuiPlayKind::Airplane { .. } => 1.38,
        _ => 1.18,
    };
    if elapsed >= duration {
        effect.active = None;
...
pub fn sequence_effect_style(
    ...
    match kind {
        QiGuiPlayKind::Straight { .. } => {
            Some(("顺子", STRAIGHT_EFFECT, SequenceEffectMotif::Wind))
        }
        QiGuiPlayKind::ConsecutivePairs { .. } => Some((
            "连对",
            CONSECUTIVE_PAIRS_EFFECT,
            SequenceEffectMotif::Flower,
        )),
        QiGuiPlayKind::Airplane { .. } => {
            Some(("飞机", AIRPLANE_EFFECT, SequenceEffectMotif::Airplane))
        }
        _ => None,
    }
}
...
pub fn animate_sequence_play_effect(
    ...
    let elapsed = active.elapsed;
    let sequence_exit = if matches!(active.play.kind, QiGuiPlayKind::Airplane { .. }) {
...
struct SequenceAirplanePose {
    pub position: Vec2,
    pub rotation: f32,
}

fn sequence_airplane_pose(progress: f32) -> SequenceAirplanePose {
...
pub fn animate_bomb_play_effect(
    ...
    let elapsed = active.elapsed;
    let explosion_at = if matches!(active.play.kind, QiGuiPlayKind::HeavenBomb) {
        0.44
    } else {
        0.52
    };
    ...
    {
        commands.spawn((
            AudioPlayer::new(assets.audio.bomb_explosion_sound.clone()),
            PlaybackSettings {
                volume: Volume::Linear(if matches!(active.play.kind, QiGuiPlayKind::HeavenBomb) {
                    0.78
                } else {
                    0.5
                }),
...
pub fn animate_heaven_bomb_play_effect(
    ...
    let Some(active) = effect
        .active
        .as_ref()
        .filter(|active| matches!(active.play.kind, QiGuiPlayKind::HeavenBomb))
    else {
        return;
    };
    let elapsed = active.elapsed;
