...
pub(in crate::app) fn add_play_effect_overlay(
    ...
    let parent = if matches!(effect.play.kind, PlayKind::HeavenBomb) {
        full_screen_parent
    } else {
        table
    };
    ...
    commands.entity(root).insert((
        PlayEffectRoot,
        Visibility::Hidden,
        GlobalZIndex(if matches!(effect.play.kind, PlayKind::HeavenBomb) {
            1900
        } else {
            1100
        }),
    ...
    match effect.play.kind {
        PlayKind::HeavenBomb => add_heaven_bomb_effect(commands, root, assets),
        PlayKind::Bomb(_) => {
...
pub(in crate::app) fn add_sequence_play_decoration(
    commands: &mut Commands,
    play_area: Entity,
    card_count: usize,
    label: &str,
...
fn add_sequence_airplane(
    ...
    let airplane = commands
        .spawn((
            SequenceAirplane,
            ImageNode::new(assets.sequence_airplane.clone()).with_color(Color::NONE),
            Node {
                width: px(62.0),
                height: px(42.0),
...
fn bomb_effect_source(game: &leocard_protocol::QiGui523Snapshot, player: PlayerId) -> Vec2 {
    let own_seat = game
        .players
        .iter()
