...
#[derive(Default)]
pub(in crate::app) struct TexasSoundAssets {
...
pub(in crate::app) enum TexasSoundKind {
    ChipLay,
    ChipHandle,
    ChipCollide,
    ChipStack,
...
pub(in crate::app) struct TexasAudioCue {
    pub(in crate::app) kind: TexasSoundKind,
    pub(in crate::app) remaining: f32,
    pub(in crate::app) volume: f32,
    pub(in crate::app) seed: u64,
}
...
pub(in crate::app) fn texas_action_sound_plan(
...
pub(in crate::app) fn texas_street_sound_plan(card_count: usize, seed: u64) -> Vec<TexasAudioCue> {
    let mut cues = vec![
...
pub(in crate::app) fn texas_hand_finish_sound_plan(
    showdown: bool,
    seed: u64,
) -> Vec<TexasAudioCue> {
    let mut cues = vec![
...
pub(in crate::app) fn texas_side_pot_sound_plan(seed: u64) -> Vec<TexasAudioCue> {
    vec![
...
pub(in crate::app) fn queue_texas_turn_sound(cues: &mut Vec<TexasAudioCue>, seed: u64) {
    cues.push(TexasAudioCue::new(TexasSoundKind::Turn, 0.04, 0.28, seed));
}

pub(in crate::app) fn play_texas_audio_cues(
    time: Res<Time>,
    assets: Res<UiAssets>,
    ...
    for cue in ready {
        let variants = assets.texas_sounds.variants(cue.kind);
