...
#[derive(Default)]
pub(in crate::app) struct UnoSoundAssets {
...
pub(in crate::app) enum UnoSoundKind {
    CardThrow,
    CardLand,
    CardDraw,
    Penalty,
...
pub(in crate::app) struct UnoAudioCue {
    pub(in crate::app) kind: UnoSoundKind,
...
pub(in crate::app) struct UnoAudioState {
...
impl UnoAudioState {
    pub(in crate::app) fn queue_event(&mut self, event: &UnoEvent, you: PlayerId) {
...
pub(in crate::app) fn uno_event_sound_plan(
    event: &UnoEvent,
    you: PlayerId,
    seed: u64,
) -> Vec<UnoAudioCue> {
...
pub(in crate::app) fn play_uno_card_selection_sounds(
    buttons: Query<&Interaction, (Changed<Interaction>, With<Button>, With<UnoHandCardButton>)>,
    assets: Res<UiAssets>,
    mut commands: Commands,
) {
    if assets.uno_sounds.select.is_empty() {
        return;
    }
    for interaction in &buttons {
        ...
        let index = fastrand::usize(..assets.uno_sounds.select.len());
        commands.spawn((
            AudioPlayer::new(assets.uno_sounds.select[index].clone()),
            PlaybackSettings {
...
pub(in crate::app) fn play_uno_audio_cues(
    time: Res<Time>,
    ...
    for cue in ready {
        let variants = assets.uno_sounds.variants(cue.kind);
