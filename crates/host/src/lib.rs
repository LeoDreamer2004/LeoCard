...
impl From<leocard_texas_holdem_adapter::AdapterError> for HostError {
    fn from(value: leocard_texas_holdem_adapter::AdapterError) -> Self {
...
/// 创建统一房主会话所需的具体游戏后端配置。
///
/// 洗牌仍由创建房间的一侧完成，方便开发者牌堆、固定种子和未来不同游戏采用
/// 各自的牌组构造方式。
#[derive(Clone, Debug)]
pub enum GameSetup {
    QiGui523 {
...
mod tests {
    use super::*;
    use leocard_protocol::{ClientCommand, QUICK_VOICE_COUNT, RuleViolation};
    ...
    fn developer_build_starts_with_an_empty_draw_pile() {
        ...
        assert_eq!(
            game.players()
                ...
                .sum::<usize>(),
            3 * usize::from(game.rules().hand_size)
        );
    }
    ...
    #[cfg(feature = "developer")]
    #[test]
    fn disabling_developer_deck_restores_the_full_draw_pile() {
        let configured = RuleSet {
            developer_deck: false,
            ..rules()
        };
        let mut session = QiGui523Session::new(ROOM, configured, build_deck(1)).unwrap();
        join_three(&mut session);
        ready_and_start(&mut session);

        assert_eq!(session.game().unwrap().draw_pile_len(), 54 - 15);
        let replacement = vec![Card::suited(
            0,
            leocard_qigui523::Suit::Spade,
            leocard_qigui523::Rank::Seven,
        )];
        ...
        let (leader, lead_card, follower, expected) = {
            ...
            game.player(leader)
                ...
                .find_map(|lead_card| {
                    ...
                    let response = GreedyStrategy::new().next_response(GreedyRequest {
